...
def add_arguments(parser):
  ...
  # network
  parser.add_argument("--num_units", type=int, default=32, help="Network size.") ###一层网络的节点个数，对应lstm 的cell个数
  parser.add_argument("--num_layers", type=int, default=2, ### 堆叠多层
                      help="Network depth.")
  parser.add_argument("--num_encoder_layers", type=int, default=None, ### encoder layers层数
                      help="Encoder depth, equal to num_layers if None.")
  parser.add_argument("--num_decoder_layers", type=int, default=None, ### decoder layers层数
                      help="Decoder depth, equal to num_layers if None.")
  ###encoder 层使用的rnn类型uni bi gnmt  gnmt是一个bi layer ，其他为 uni layer  组成的encoder
  ...
  ### emebedding partitions  这个是做什么？
  parser.add_argument("--num_embeddings_partitions", type=int, default=0,
                      help="Number of partitions for embedding vars.")

  # attention mechanisms
  ####注意力机制类型，  luong | scaled_luong | bahdanau | normed_bahdanau
  parser.add_argument("--attention", type=str, default="", help="""\
      luong | scaled_luong | bahdanau | normed_bahdanau or set to "" for no
      attention\
      """)
  ###attention_architecture attention 结构，指怎么计算attention？
  parser.add_argument(
...
####训练次数
  ...
  # initializer
  ###初始化uniform,glorot normal,glorot uniform
  ...
  ###使用sampled_softmax_loss 还是full softmax loss
...
###use_char_encode 字符级别
  ...
  ###打印信息log_device_placement
  parser.add_argument("--log_device_placement", type="bool", nargs="?",
                      const=True, default=False, help="Debug GPU allocation.")
  ###evaluation指标(bleu,rouge,accuracy)
  ...
  ###infer mode "greedy", "sample", "beam_search" beam search 最强大
...
def create_or_load_hparams(
  ...
  hparams = utils.load_hparams(out_dir)###加载hparams
  if not hparams:
...
def run_main(flags, default_hparams, train_fn, inference_fn, target_session=""):
  ...
  if random_seed is not None and random_seed > 0:
    ...
    random.seed(random_seed + jobid) ###不同的job设置不同的 random seed
    np.random.seed(random_seed + jobid)

  # Model output directory 创建输出目录
  out_dir = flags.out_dir
  ...
  ## Train / Decode
  if flags.inference_input_file: ###inference
...
def main(unused_argv):
  default_hparams = create_hparams(FLAGS) ###从配置中得到？
  train_fn = train.train ###train方法
  inference_fn = inference.inference###inference方法
  run_main(FLAGS, default_hparams, train_fn, inference_fn) ###四个参数，FLAGS，default_hparams


if __name__ == "__main__":
  nmt_parser = argparse.ArgumentParser() ####在调用时输入的超参数
  add_arguments(nmt_parser) ###取得输入的超参数
  FLAGS, unparsed = nmt_parser.parse_known_args() ###获得超参数
