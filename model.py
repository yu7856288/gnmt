...
class BaseModel(object):
  ...
  def __init__(self,
    ...
    Args:
      ...
      iterator: Dataset Iterator that feeds data. tf.data iterator
      source_vocab_table: Lookup table mapping source words to ids. 将源词汇转为id
      target_vocab_table: Lookup table mapping target words to ids. 将目标词汇转为id
      reverse_target_vocab_table: Lookup table mapping ids to target words. Only
        required in INFER mode. Defaults to None. 从id找到目标词汇
    ...
    self.saver = tf.train.Saver(
        tf.global_variables(), max_to_keep=hparams.num_keep_ckpts)###ckpts的次数
  ...
  def _set_params_initializer(self,
    ...
    self.num_sampled_softmax = hparams.num_sampled_softmax ####sampled softmax

    # extra_args: to make it flexible for adding external customizable code
    self.single_cell_fn = None
    if extra_args:
      self.single_cell_fn = extra_args.single_cell_fn ###single cell fn 函数对象

    # Set num units
    self.num_units = hparams.num_units

    # Set num layers
    self.num_encoder_layers = hparams.num_encoder_layers ####encoder 层数
    self.num_decoder_layers = hparams.num_decoder_layers ### decoder 层数
    assert self.num_encoder_layers
    assert self.num_decoder_layers

    # Set num residual layers
    if hasattr(hparams, "num_residual_layers"):  # compatible common_test_utils
      self.num_encoder_residual_layers = hparams.num_residual_layers ### residual layers  序列网络也能设residual layer
      self.num_decoder_residual_layers = hparams.num_residual_layers
    else:###没有encoder residual layer 的情况，只有 encoder 和decoder
      self.num_encoder_residual_layers = hparams.num_encoder_residual_layers
      self.num_decoder_residual_layers = hparams.num_decoder_residual_layers

    # Batch size
    self.batch_size = tf.size(self.iterator.source_sequence_length) ####batch size 设置为 source sequence length?
    ...
    if extra_args and extra_args.encoder_emb_lookup_fn:
      self.encoder_emb_lookup_fn = extra_args.encoder_emb_lookup_fn #### 自定义的 embedding lookup
    else:
      self.encoder_emb_lookup_fn = tf.nn.embedding_lookup ### 使用默认的 embedding lookup
    self.init_embeddings(hparams, scope)

  def _set_train_or_infer(self, res, reverse_target_vocab_table, hparams):
    """Set up training and inference."""
    if self.mode == tf.contrib.learn.ModeKeys.TRAIN:   ###定义的三个常量
                                                                        # TRAIN = 'train'
                                                                        # EVAL = 'eval'
                                                                        # INFER = 'infer'
      ### train  eval infer 不同
    ...
    if self.mode == tf.contrib.learn.ModeKeys.TRAIN:
      ...
      # Gradients
      ###colocate_gradients_with_ops 用在并发？
      gradients = tf.gradients(
          self.train_loss,
          params,
          colocate_gradients_with_ops=hparams.colocate_gradients_with_ops)
      ###clipped grads 梯度裁剪和梯度更新
