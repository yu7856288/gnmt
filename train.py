...
def get_model_creator(hparams):
  ...
  if (hparams.encoder_type == "gnmt" or
    ...
    model_creator = gnmt_model.GNMTModel ####  按超参数设置，取GNMTMdel
  elif hparams.attention_architecture == "standard":
    model_creator = attention_model.AttentionModel ###standard 为 AttentionModel
  elif not hparams.attention:
    model_creator = nmt_model.Model ####取base的 Model
  else:
...
def train(hparams, scope=None, target_session=""):
  ...
  steps_per_eval = 10 * steps_per_stats ####评估是step per stats的10倍
  avg_ckpts = hparams.avg_ckpts ###avg ckpts 标志

  if not steps_per_external_eval:
    steps_per_external_eval = 5 * steps_per_eval ####没有设steps_per_external_eval，则取5倍

  # Create model
  ###创建train eval infer 三个model
  ###model creator 会根据hparams中的模型类型设置得到不同类型的模型：(gnmt)gnmt_model,(standard)attention_model, (base) model
  model_creator = get_model_creator(hparams)
  ###三个含有的属性，graph，model，iterator，
  train_model = model_helper.create_train_model(model_creator, hparams, scope) ###scope用于设置参数的scope
  ...
  ####分别设置train eval 和infer的session，使用不同的graph
  train_sess = tf.Session( ###Session 的config的配置
...
def _sample_decode(model, global_step, sess, hparams, iterator, src_data,
  ...
  decode_id = random.randint(0, len(src_data) - 1) ###随机选择一个句子
  ...
  sess.run(iterator.initializer, feed_dict=iterator_feed_dict) ###运行sess

  nmt_outputs, attention_summary = model.decode(sess) ###做decode

  if hparams.infer_mode == "beam_search":
    # get the top translation.
    nmt_outputs = nmt_outputs[0] ###beam search 取第一个值
...
###将id转为句子
  ...
  utils.print_out(b"    nmt: " + translation) ###输出的句子
