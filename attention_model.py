...
class AttentionModel(model.Model):
  ...
  def __init__(self,
               ...
               scope=None,
               extra_args=None):
    self.has_attention = hparams.attention_architecture and hparams.attention ###attention参数 和attention architecture 参数

    # Set attention_mechanism_fn
    if self.has_attention:
      if extra_args and extra_args.attention_mechanism_fn:
        self.attention_mechanism_fn = extra_args.attention_mechanism_fn ###有外部指定的attention mechanism方法
      else:
        self.attention_mechanism_fn = create_attention_mechanism ####使用AttentionModel内定义的 attention mechanism
    ###父类初始化
...
###build decoder cell，根据是否要attention mechanism，做不同的decoder cell
  def _build_decoder_cell(self, hparams, encoder_outputs, encoder_state,
                          source_sequence_length):
    """Build a RNN cell with attention mechanism that can be used by decoder."""
    # No Attention
    if not self.has_attention: ###没有attention，则是base model
      return super(AttentionModel, self)._build_decoder_cell(
          hparams, encoder_outputs, encoder_state, source_sequence_length)
    elif hparams.attention_architecture != "standard": ####attention  architecture
      raise ValueError(
          "Unknown attention architecture %s" % hparams.attention_architecture)

    num_units = hparams.num_units
    num_layers = self.num_decoder_layers
    num_residual_layers = self.num_decoder_residual_layers ###带residual layers
    infer_mode = hparams.infer_mode

    dtype = tf.float32

    # Ensure memory is batch-major
    #batch major 和time major
    if self.time_major:
 ...
 ###创建rnn cell，这个是基本的cell
    cell = model_helper.create_rnn_cell(
        ...
        mode=self.mode,
        single_cell_fn=self.single_cell_fn) ###指定single cell fn

    # Only generate alignment in greedy INFER mode.
    alignment_history = (self.mode == tf.contrib.learn.ModeKeys.INFER and
                         infer_mode != "beam_search") ###在beam search 不做alignment？
    ###这个cell 包裹了一个AttentionWrapper
    ###AttentionWrapper 需要一个cell参数和attention mechanism 参数
    ...
    ###是否使用 ecoder state到decoder
    if hparams.pass_hidden_state:
      decoder_initial_state = cell.zero_state(batch_size, dtype).clone(
          cell_state=encoder_state)
    else:
      decoder_initial_state = cell.zero_state(batch_size, dtype) ###用zero 初始化

    return cell, decoder_initial_state ###的奥decoder intial state
...
####自定义的创建attention mechanism 根据不同超参数，创建不同的mechanism
